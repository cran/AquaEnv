...
+     with(as.list(c(currentstate, parameters)), {
+         t <- c(seq(t_min, t_max, (t_max - t_min)/(modeltime/2)), 
+             seq(t_max, t_min, -(t_max - t_min)/(modeltime/2)))[[round(timestep) + 
+             1]]
+         ae <- aquaenv(S = S, t = t, SumCO2 = SumCO2, SumNH4 = SumNH4, 
+             TA = TA)
...
+             RPP <- rPP * ((SumNH4 + NO3)/(ksDINPP + (SumNH4 + 
+                 NO3)))
...
+         dO2 <- TO2 + EO2 - ROxCarbon - 2 * RNit + (2 - 2 * pNH4PP) * 
+             RPP + RPPCarbon
...
+         transport <- c(TO2 = TO2, TNO3 = TNO3, TSumNH4 = TSumNH4, 
+             TTA = TTA, TSumCO2 = TSumCO2)
+         airseaexchange <- c(ECO2 = ECO2, EO2 = EO2)
+         return(list(ratesofchanges, list(transport, airseaexchange, 
+             ae)))
+     })
+ }
\end{Sinput}
...
+     initialstate <<- c(O2 = O2_io, NO3 = NO3_io, SumNH4 = SumNH4_io, 
+         SumCO2 = SumCO2_io, TA = TA_io)
+     times <<- c(0:modeltime)
...
+         ratesofchanges <- c(dO2, dNO3, dSumNH4, dSumCO2, dTA, 
+             dH, dH_stoich)
+         processrates <- c(ECO2 = ECO2, EO2 = EO2, RNit = RNit, 
+             ROx = ROx, RPP = RPP)
+         DSA <- c(DSApH = DSApH, rhoHECO2 = rhoHECO2, rhoHRNit = rhoHRNit, 
+             rhoHROx = rhoHROx, rhoHRPP = rhoHRPP, dH_ECO2 = dH_ECO2, 
+             dH_RNit = dH_RNit, dH_ROx = dH_ROx, dH_RPP = dH_RPP, 
+             DSAstoichpH = DSAstoichpH)
...
\begin{Sinput}
> with(as.list(parameters), {
+     H_init <<- 10^(-(aquaenv(S = S, t = t, SumCO2 = SumCO2_io, 
+         SumNH4 = SumNH4_io, TA = TA_io, speciation = FALSE)$pH))
+     initialstate <<- c(O2 = O2_io, NO3 = NO3_io, SumNH4 = SumNH4_io, 
+         SumCO2 = SumCO2_io, TA = TA_io, H = H_init, H_stoich = H_init)
+     times <<- c(0:modeltime)
+     output <<- as.data.frame(vode(initialstate, times, boxmodel, 
+         parameters, hmax = 1))[-1, ]
+ })
...
> boxmodel <- function(timestep, currentstate, parameters) {
+     with(as.list(c(currentstate, parameters)), {
+         ae <- aquaenv(S = S, t = t, SumCO2 = SumCO2, TA = TA, 
+             SumSiOH4 = 0, SumBOH3 = 0, SumH2SO4 = 0, SumHF = 0)
...
> boxmodel <- function(timestep, currentstate, parameters) {
+     with(as.list(c(currentstate, parameters)), {
+         ae <- aquaenv(S = S, t = t, SumCO2 = SumCO2, pH = -log10(H), 
+             SumSiOH4 = 0, SumBOH3 = 0, SumH2SO4 = 0, SumHF = 0, 
+             dsa = TRUE)
+         Rc <- kc * ((ae$CO2_sat) - (ae$CO2))
+         Rp <- kp * (1 - ae$omega_calcite)^n
...
+         return(list(ratesofchanges, list(processrates, outputvars, 
+             ae)))
+     })
+ }
\end{Sinput}
...
\begin{Sinput}
> with(as.list(parameters), {
+     aetmp <- aquaenv(S = S, t = t, SumCO2 = SumCO2_t0, TA = TA_t0, 
+         SumSiOH4 = 0, SumBOH3 = 0, SumH2SO4 = 0, SumHF = 0)
+     H_t0 <- 10^(-aetmp$pH)
...
+         ratesofchanges <- c(dSumCO2, dH)
+         processrates <- c(Rc = Rc, Rp = Rp)
+         outputvars <- c(dHRc = dHRc, dHRp = dHRp, rhoc = rhoc, 
+             rhop = rhop)
+         return(list(ratesofchanges, list(processrates, outputvars, 
+             ae)))
+     })
...
\begin{Schunk}
\begin{Sinput}
> simpletitration <- function(aquaenv, volume, amount, steps, type) {
+     directionTAchange <- switch(type, HCl = -1, NaOH = +1)
+     TAconcchangeperstep <- convert(((amount/steps)/volume), "conc", 
+         "molar2molin", aquaenv$t, aquaenv$S)
+     aquaenvtemp <- aquaenv
+     for (i in 1:steps) {
...
+     }
+     aquaenv[["DeltaCTitrant"]] <- convert((amount/volume)/steps * 
+         (1:(steps + 1)), "conc", "molar2molin", aquaenv$t, aquaenv$S)
+     return(aquaenv)
+ }
\end{Sinput}
\end{Schunk}
